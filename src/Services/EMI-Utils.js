// Services/EMI-Utils.js - EMI Calculations
export const getEmiDetails = (course) => {
  ...
  if (!isEmiAvailable) {
    // console.log("[2] getEmiDetails: EMI not available for this course", {
    //   emiConfig: course?.emi,
    //   isAvailable: course?.emi?.isAvailable,
    //   type: typeof course?.emi?.isAvailable,
    // });
    return {
      eligible: false,
      months: 0,
  ...
  //console.log("[2] getEmiDetails result:", result);
  return result;
};

export const validateCourseForEmi = (course) => {
  // console.log(
  //   "[1] validateCourseForEmi called with course:",
  //   course?.coursename || "unknown"
  // );

  // Check if course has EMI configuration
  if (!course?.emi) {
    //console.log("[2] validateCourseForEmi: No EMI configuration found");
    throw new Error("EMI configuration not found for this course");
  }

  const emiDetails = getEmiDetails(course);
  //console.log("[3] validateCourseForEmi emiDetails:", emiDetails);

  if (!emiDetails.eligible) {
    //console.log("[4] validateCourseForEmi: Not eligible for EMI");
  ...
  if (course.price.finalPrice !== emiDetails.totalAmount) {
    // console.log(
    //   "[5] validateCourseForEmi: Price mismatch",
    //   "Course finalPrice:",
  ...
  //console.log("[6] validateCourseForEmi: Course is valid for EMI");
  return emiDetails;
};

// Calculate EMI payment status and due amounts
export const calculateEmiStatus = (emiPlan) => {
  //console.log("[1] calculateEmiStatus called for plan:", emiPlan._id);
  ...
  // console.log("[2] calculateEmiStatus result:", {
  //   totalPaid: result.totalPaid,
  //   totalOverdue: result.totalOverdue,
  //   totalRemaining: result.totalRemaining,
  //   hasAccessToContent: result.hasAccessToContent,
  // });
...
export const calculatePaymentAllocation = (emiPlan, paymentAmount) => {
  // console.log("[1] calculatePaymentAllocation called:", {
  //   planId: emiPlan._id,
  //   paymentAmount,
  // });
  ...
  // console.log("[2] calculatePaymentAllocation result:", {
  //   isValidAmount: result.isValidAmount,
  //   totalAllocated: result.totalAllocated,
  //   emisCount: result.emisToPay.length,
  // });
...
export const updateEmiAfterPayment = async (
  ...
  // console.log("[1] updateEmiAfterPayment called:", {
  //   planId: emiPlan._id,
  //   emisToPayCount: paymentAllocation.emisToPay.length,
  // });
  ...
  // console.log("[2] updateEmiAfterPayment completed:", {
...
export const createEmiPaymentRecord = async (
  ...
  // console.log("[1] createEmiPaymentRecord called:", {
  ...
  // console.log("[2] createEmiPaymentRecord completed:", payment._id);
  return payment;
};
