...
export const getCourseDetailWithPaymentCheck = async (req, res) => {
  try {
    ...
    if (accessInfo.hasAccess && accessInfo.accessType === "full") {
      // Full access - return complete course details
      const courseData = { ...course.toObject(), emi: formatEmiData(course) };
      return res.status(200).json({
        success: true,
        data: courseData,
        access: "full",
      ...
      // Limited access - return basic details for non-paying users or overdue EMI users
      const basicDetails = {
        _id: course._id,
...
export const getCourseContent = async (req, res) => {
  try {
    ...
    if (accessInfo.hasAccess && accessInfo.accessType === "full") {
      // Full access - return complete content
      return res.status(200).json({
        success: true,
